...
- Every managed browser must have an owner id, so a start can never produce a browser nothing can stop. Library callers pass `ownerId`; a command-line start stamps `cli` when none is given. A browser that nothing owns is treated as reclaimable garbage by `stop --reap` and can be stopped without a token, while an owned browser always needs its own.
